...
class ObjectiveFunction(object):
    ...
    def __init__(self, target_eval, max_eval, dim):
        self.target_eval = target_eval
        self.max_eval = max_eval
        self.dim = dim
        self.bestX = np.ones(dim) * np.inf  # bestXを初期化
    ...
    def LST_transformation(self, x, s):
        a = np.eye(x.size)
        b = np.eye(x.size) * -1
        c = np.zeros([1, x.size])
        Delta = np.concatenate([a, b, c], axis=0)
        F_lst = np.array([self.__call__(np.array([x + s * delta])) for delta in Delta])
        
        # F_lst.max() と eval_c の両方を配列で返す
        return np.array([F_lst.max(), F_lst.min()])

    def clear(self):
        self.eval_count = 0
        self.best_eval = np.inf if self.minimization_problem else -np.inf
        self.bestX = np.ones(self.dim) * np.inf
    ...
    def _update_best_eval(self, evals, x):
        """
        ...
        if self.best_eval == self.get_best(evals):
            # 修正例
            if evals.ndim == 0:
                self.bestX = x
            else:
                best_idx = np.argmax(evals) if self.minimization_problem else np.argmin(evals)
                self.bestX = x[best_idx]  # 最良の評価値に対応するインデックスを使用
